...
<template>
    <div class="container" style="margin-top:90px;">
        ...
        <form @submit.prevent  style="width:100%;">
            ...
            <br>
            <h2 class="text-center" style="color:#62ab00; font-size: 30px;">Chỉnh sửa thể loại sách</h2>
            <!-- Title Input -->
            <div class="mb-3">
                <label style="font-size: 20px;" for="exampleInputEmail1" class="form-label fw-bold">Tên thể loại: </label>
                <input style="font-size: 20px;" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Vui lòng nhập vào thể loại..." v-model="BookTypeInput.booktype_name" required>

                <br>
                <label style="font-size: 20px;" for="exampleInputEmail1" class="form-label fw-bold">Chọn danh mục: </label>
                <br>
                <select style="font-size: 20px;" class="form-control" :required="true" @change="changeOption" id="chooseDM">
                    <option v-for="item in ManageCategory" v-bind:value="item.category_name">{{ item.category_name }}</option>
                </select>
            </div>

            <button @click="handleEditBookType()" type="submit" class="btn btn-light fw-bold" style="padding: 10px;;border:1px solid #ccc; font-size: 20px;">Xác Nhận</button>
        </form>
