...
<template>
    <br>
    <!-- <button type="button" id="btn_addBook" class="btn btn-success" @click="addBook()" style="position: relative; left: 22%;"><i class="fa-solid fa-plus" ></i> Thêm Sách </button> -->


    <form action="/" class="d-flex" style="width:500px;margin:0 auto" id="inputSearchKeyWord" @submit.prevent>
        <input type="text" class="form-control" placeholder="Nhập vào tên sách hoặc mô tả..." aria-label="Recipient's username" aria-describedby="basic-addon2" style="border-radius:6px; font-size: 15px; position: absolute; top: 40%; width: 30%;"
            v-model="inputSearch">
        <button id="btn_search" type="submit" style="width:120px;height:40px;border-radius:10px; font-size: 15px; position: absolute; top: 39%; left: 57%;" @click="FindBook(inputSearch)">Tìm Kiếm</button>
    </form>
    <br>
    <br>
    <!-- Giao diện thêm sách -->
